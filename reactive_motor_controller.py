...
# Feature Activation
################################
FEATURE_STOP_SIGN = ON
FEATURE_TELEMETRY = ON
FEATURE_HW_MONITOR = ON
...
STOP_WAIT = 5
EMERSON_CONSTANT = 0.4

SPEED_MULTIPLIER = 1
ANGLE_MULTIPLIER = 0.85

# THRESHOLD = 0.5
STOP_THRESHOLD = 0.6 # if any lidar beam returns a distance less than this amount, STOP
HALLWAY_THRESHOLD = 3.25 # makes sure both lidar_E and lidar_W are under this distance for hallway state
HALLWAY_END_THRESHOLD = 3.0 # drive while staying centered until lidar_N reaches this distance
TURN_THRESHOLD = 1.15 * HALLWAY_END_THRESHOLD # when the lidar_N see's this distance, turn right
# the turning threshold is slightly lower than the hallway's to let the robot drive forward a bit before turning

THROTTLE = 0.75 * SPEED_MULTIPLIER
if FEATURE_BACKWARD_DRIVING == ON:
    ...
    def listener_callback(self, msg):
        ...
        if self.reverse_drive == 0:
            ...
            if dangerously_close:
                ...
                self.motor_pub.start_motor_forward(0.85*THROTTLE)
            # self.stopped = 1
            ...
            elif lidar_N <= TURN_THRESHOLD and ((lidar_NE > lidar_N) or (lidar_E > HALLWAY_THRESHOLD)):
                self.motor_pub.current_throttle = 1.0 * THROTTLE
                self.motor_pub.set_angle(-0.8)
            ...
            # in this case, try and keep the robot centered by making small turns depending on proximity to walls
                ...
                if lidar_E - EMERSON_CONSTANT < lidar_W + EMERSON_CONSTANT:
                    # publish message
                    self.motor_pub.set_angle(0.25 * ANGLE_MULTIPLIER)
                ...
                elif lidar_E - EMERSON_CONSTANT > lidar_W + EMERSON_CONSTANT:
                    # publish message
                    self.motor_pub.set_angle(-0.15 * ANGLE_MULTIPLIER)
        ...
        # reverse driving is the same as forward, but with different lidar beams
            ...
            # this doesn't check that the right is another hallway
            # if going down a corridor with no turns, then this behavior will cause it to u-turn or stop depending on its speed
            if lidar_S <= TURN_THRESHOLD and ((lidar_SW > lidar_S) or (lidar_W > HALLWAY_THRESHOLD)):
                self.motor_pub.current_throttle = 1.0 * -THROTTLE
                self.motor_pub.set_angle(0.4)
                self.get_logger().info("found turn condition. turning right")
                self.current_state = "Reverse Right Turn"
            ...
            # in this case, try and keep the robot centered by making small turns depending on proximity to walls
                ...
                if lidar_E + EMERSON_CONSTANT > lidar_W - EMERSON_CONSTANT:
                    ...
                    self.motor_pub.set_angle(-0.25 * ANGLE_MULTIPLIER)
                    # self.get_logger().info("turning slightly left")
                    self.current_state = "Reverse Slight Left"
                # if the robot is closer to left wall, turn slightly right
                elif lidar_E + EMERSON_CONSTANT < lidar_W - EMERSON_CONSTANT:
                    # publish message
                    self.motor_pub.set_angle(0.125 * ANGLE_MULTIPLIER)
                    # self.get_logger().info("turning slightly right")
                    self.current_state = "Reverse Slight Right"

            # if none of the other states are active, simply drive forward
            else:
                self.motor_pub.start_motor_backward()
                # self.get_logger().info("no other states active. driving forward")
...
class CameraSubscriber(Node):
    ...
    def listener_callback(self, msg : Image):
        ...
        while i < len(msg.data):
            ...
            i+=3
        if self.num_color > 500:
            self.get_logger().info("green values %d " % self.num_color)
...
def main(args=None):
    ...
    print("Spinning")
    if FEATURE_BACKWARD_DRIVING == OFF:
    ...
    while(1):
        ...
        if lidar_subscriber.stopped == 1:
            ...
            break
        if FEATURE_STOP_SIGN == ON:
            if camera_subscriber.has_stopped == 0:
